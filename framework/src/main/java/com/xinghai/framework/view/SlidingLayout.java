 ...
 *
 * {@link #setSlidingOffset(float slidingOffset)} 设置控件的滑动阻力，有效值为0.1F~1.0F，值越小阻力越大，默认为0.5F
 *
 * {@link #setTargetView(View view)} 设置控件的前景View
 * {@link #setBackgroundView(View view)} 设置控件的背景View
 * {@link #setSlidingListener(SlidingListener slidingListener)} 给控件设置监听，可以监听滑动情况
 * {@link #setSlidingMode(int mode)} 设置滑动模式
 * {@link #setSlidingDistance(int max)} 设置最大滑动距离，仅在top模式下有效
 *
 */
