...
/**
 * 继承ViewGroup实现流式布局
 * @see #setHorizontalSpacing(float) 设置子控件间的水平间隔
 * @see #setVerticalSpacing(float) 设置行间隔
 * 
 * @author xinghai
 *
 */
public class FlowLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			childWidth = child.getMeasuredWidth() + mp.leftMargin + mp.rightMargin;

			childHeight = child.getMeasuredHeight() + mp.topMargin + mp.bottomMargin;
		...
		setMeasuredDimension(
				widthMode == MeasureSpec.AT_MOST ? maxLineWidth + getPaddingLeft() + getPaddingRight() : widthMeasure,
				heightMode == MeasureSpec.AT_MOST ? totalHeight + getPaddingTop() + getPaddingBottom() : heightMeasure);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (curLeft + childWidth > rightLimit ) {
				...
				viewT = curTop + params.topMargin + lineNum * (int)mVerticalSpacing;
				viewR = viewL + childW;
				viewB = viewT + childH;
				...
				//当前行可以放下子View:
				viewL = i == 0 ? curLeft +params.leftMargin : curLeft +params.leftMargin + (int)mHorizontalSpacing;
				Log.d("onlayout行数:",lineNum+""+"\t"+i);
				viewT = curTop + params.topMargin + lineNum * (int)mVerticalSpacing;
				viewR = viewL + childW;
				viewB = viewT + childH;
